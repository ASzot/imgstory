//
//  AMWFindFriendsViewController.m
//  ParseStarterProject
//
...
typedef enum {
    AMWFindFriendsFollowingNone = 0,    // User isn't following anybody in Friends list
    AMWFindFriendsFollowingAll,         // User is following all Friends
    AMWFindFriendsFollowingSome         // User is following some of their Friends
} AMWFindFriendsFollowStatus;

@interface AMWFindFriendsViewController ()

@property (nonatomic, strong) UIView *headerView;
@property (nonatomic, assign) AMWFindFriendsFollowStatus followStatus;
...
@implementation AMWFindFriendsViewController
...
- (id)initWithStyle:(UITableViewStyle)style {
    ...
    if (self) {
        ...
        self.followStatus = AMWFindFriendsFollowingSome;
    }
    return self;
}
...
- (PFQuery *)queryForTable {
    ...
    NSMutableArray *parseEmployees = [[NSMutableArray alloc] initWithArray:kAMWParseEmployeeAccounts];
    [parseEmployees removeObject:[[PFUser currentUser] objectForKey:kAMWUserFacebookIDKey]];
    PFQuery *parseEmployeeQuery = [PFUser query];
    [parseEmployeeQuery whereKey:kAMWUserFacebookIDKey containedIn:parseEmployees];
    
    [parseEmployeeQuery orderByAscending:kAMWUserDisplayNameKey];
    
    return parseEmployeeQuery;
}
...
- (void)objectsDidLoad:(NSError *)error {
    ...
    [isFollowingQuery whereKey:kAMWActivityToUserKey containedIn:self.objects];
    [isFollowingQuery setCachePolicy:kPFCachePolicyNetworkOnly];
    
    [isFollowingQuery countObjectsInBackgroundWithBlock:^(int number, NSError *error) {
        if (!error) {
            if (number == self.objects.count) {
                self.followStatus = AMWFindFriendsFollowingAll;
                ...
                self.followStatus = AMWFindFriendsFollowingNone;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath object:(PFObject *)object {
    ...
    if (self.followStatus == AMWFindFriendsFollowingSome) {
        if (attributes) {
        ...
        [cell.followButton setSelected:(self.followStatus == AMWFindFriendsFollowingAll)];
    }
...
#pragma mark - AMWFindFriendsCellDelegate
...
- (void)followAllFriendsButtonAction:(id)sender {
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        ...
        for (int r = 0; r < self.objects.count; r++) {
            ...
            AMWFindFriendsCell *cell = (AMWFindFriendsCell *)[self tableView:self.tableView cellForRowAtIndexPath:indexPath object:user];
        ...
        [AMWUtility followUsersEventually:self.objects block:^(BOOL succeeded, NSError *error) {
...
- (void)unfollowAllFriendsButtonAction:(id)sender {
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        ...
        [AMWUtility unfollowUsersEventually:self.objects];
...
- (void)presentMailComposeViewController:(NSString *)recipient {
    ...
    [composeEmailViewController setSubject:@"Join me on MyWall"];
    [composeEmailViewController setToRecipients:[NSArray arrayWithObjects:recipient, nil]];
    [composeEmailViewController setMessageBody:@"<h2>Share your pictures, share your story.</h2><p>Easy way to share pictures with your friends.</p>" isHTML:YES];
...
- (void)presentMessageComposeViewController:(NSString *)recipient {
    ...
    [composeTextViewController setBody:@"Check out MyWall!"];
