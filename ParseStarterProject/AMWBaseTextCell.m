...
+ (CGFloat)horizontalTextSpaceForInsertWidth:(CGFloat)insertWidth;
@end


@implementation AMWBaseTextCell
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        horizontalTextSpace =  [AMWBaseTextCell horizontalTextSpaceForInsetWidth:cellInsetWidth];
...
#pragma mark - AMWBaseTextCell

/* Static helper to get the height for a cell if it had the given name and content */
+ (CGFloat)heightForCellWithName:(NSString *)name contentString:(NSString *)content {
    return [AMWBaseTextCell heightForCellWithName:name contentString:content cellInsetWidth:0];
}
...
+ (CGFloat)heightForCellWithName:(NSString *)name contentString:(NSString *)content cellInsetWidth:(CGFloat)cellInset {
    ...
    NSString *paddedString = [AMWBaseTextCell padString:content withFont:[UIFont systemFontOfSize:13] toWidth:nameSize.width];
    CGFloat horizontalTextSpace = [AMWBaseTextCell horizontalTextSpaceForInsetWidth:cellInset];
...
- (void)setUser:(PFUser *)aUser {
    ...
    if ([AMWUtility userHasProfilePictures:self.user]) {
        [self.avatarImageView setFile:[self.user objectForKey:kAMWUserProfilePicSmallKey]];
    } else {
        [self.avatarImageView setImage:[AMWUtility defaultProfilePicture]];
    }
    
    [self.nameButton setTitle:[self.user objectForKey:kAMWUserDisplayNameKey] forState:UIControlStateNormal];
    [self.nameButton setTitle:[self.user objectForKey:kAMWUserDisplayNameKey] forState:UIControlStateHighlighted];
...
- (void)setContentText:(NSString *)contentString {
    ...
    if (self.user) {
        CGSize nameSize = [self.nameButton.titleLabel.text boundingRectWithSize:CGSizeMake(nameMaxWidth, CGFLOAT_MAX)
                                                                     ...
                                                                     attributes:@{NSFontAttributeName:[UIFont boldSystemFontOfSize:13.0f]}
                                                                        context:nil].size;
        NSString *paddedString = [AMWBaseTextCell padString:contentString withFont:[UIFont systemFontOfSize:13] toWidth:nameSize.width];
...
- (void)setCellInsetWidth:(CGFloat)insetWidth {
    ...
    horizontalTextSpace = [AMWBaseTextCell horizontalTextSpaceForInsetWidth:insetWidth];
    [self setNeedsDisplay];
}
...
- (id<AMWBaseTextCellDelegate>)delegate {
    return (id<AMWBaseTextCellDelegate>)delegate;
}

- (void)setDelegate:(id<AMWBaseTextCellDelegate>)aDelegate {
