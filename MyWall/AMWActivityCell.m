...
@interface AMWBaseTextCell ()

/*! Private view components */
@property (nonatomic, strong) AMWProfileImageView *activityImageView;
...
@implementation AMWActivityCell
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        horizontalTextSpace = [AMWActivityCell horizontalTextSpaceForInsetWidth:0];
        ...
        self.activityImageView = [[AMWProfileImageView alloc] init];
...
- (void)setActivity:(PFObject *)activity {
    ...
    _activity = activity;
    if ([[activity objectForKey:kAMWActivityTypeKey] isEqualToString:kAMWActivityTypeFollow] || [[activity objectForKey:kAMWActivityTypeKey] isEqualToString:kAMWActivityTypeJoined]) {
        [self setActivityImageFile:nil];
    } else {
        [self setActivityImageFile:(PFFile*)[[activity objectForKey:kAMWActivityPhotoKey] objectForKey:kAMWPhotoThumbnailKey]];
    }
    
    NSString *activityString = [AMWActivityFeedViewController stringForActivityType:(NSString*)[activity objectForKey:kAMWActivityTypeKey]];
    self.user = [activity objectForKey:kAMWActivityFromUserKey];
    ...
    if (self.user && [self.user objectForKey:kAMWUserDisplayNameKey] && [[self.user objectForKey:kAMWUserDisplayNameKey] length] > 0) {
        nameString = [self.user objectForKey:kAMWUserDisplayNameKey];
    }
    ...
    if (self.user) {
        CGSize nameSize = [self.nameButton.titleLabel.text boundingRectWithSize:CGSizeMake(nameMaxWidth, CGFLOAT_MAX)
                                                                     ...
                                                                     attributes:@{NSFontAttributeName:[UIFont boldSystemFontOfSize:13.0f]}
                                                                        context:nil].size;
        NSString *paddedString = [AMWBaseTextCell padString:activityString withFont:[UIFont systemFontOfSize:13.0f] toWidth:nameSize.width];
...
- (void)setCellInsetWidth:(CGFloat)insetWidth {
    ...
    horizontalTextSpace = [AMWActivityCell horizontalTextSpaceForInsetWidth:insetWidth];
}
...
- (id<AMWActivityCellDelegate>)delegate {
    return (id<AMWActivityCellDelegate>)_delegate;
}

- (void)setDelegate:(id<AMWActivityCellDelegate>)delegate {
...
+ (CGFloat)heightForCellWithName:(NSString *)name contentString:(NSString *)content cellInsetWidth:(CGFloat)cellInset {
    CGSize nameSize = [name boundingRectWithSize:CGSizeMake(200.0f, CGFLOAT_MAX)
                                      ...
                                      attributes:@{NSFontAttributeName:[UIFont boldSystemFontOfSize:13.0f]}
                                         context:nil].size;
    NSString *paddedString = [AMWBaseTextCell padString:content withFont:[UIFont systemFontOfSize:13.0f] toWidth:nameSize.width];
    CGFloat horizontalTextSpace = [AMWActivityCell horizontalTextSpaceForInsetWidth:cellInset];
